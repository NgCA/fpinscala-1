...
We are calling `foldRight` with the `B` type being instantiated to `B => B`, then calling the built up function with the `z` argument. Try expanding
the definitions by substituting equals for equals using a simple example, like `foldLeft(List(1,2,3), 0)(_ + _)` if this isn't clear. Note these
...
/*
innerF = (g,a) => (b => g(f(a,b)))
foldRightViaFoldLeft(List(1,2,3), 0)(_ + _)

foldLeft(List(1,2,3), (b:B) => b)((g,a) => b => g(f(a,b)))(0)
foldLeft(List(2,3), ((b:B) => b, 1) => b => g(f(1, b))))((g,a) => b => g(f(a,b)))(0)
foldLeft(List(2,3), ((b:B) => b, 1) => b => 1 + b))((g,a) => b => g(f(a,b)))(0)
foldLeft(List(2,3), b => 1 + b)((g,a) => b => g(f(a,b)))(0)
foldLeft(List(3), (b => 1 + b, 2) => b => g(2 + b))((g,a) => b => g(f(a,b)))(0)
foldLeft(List(3), b => 1 + (2+b))((g,a) => b => g(f(a,b)))(0)
foldLeft(Nil, (b => 1+(2+b), 3) => b => g(3,b))((g,a) => b => g(f(a,b)))(0)
foldLeft(Nil, b => 1+(2+(3+b)))((g,a) => b => g(f(a,b)))(0)
1+(2+(3+b))(0)
1+(2+(3+0))
1+(2+3)
1+5
